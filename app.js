const SONGS=[
 ...
 {title:'Humko Humise Chura Lo',artist:'Lata Mangeshkar • Udit Narayan',id:'zWPsjhBaRb0',mood:'Romance'},
 ...
 {title:'Do Dil Mil Rahe Hain',artist:'Kumar Sanu',id:'Iit_5etxDkk',mood:'90s Romance'}
];
let library=SONGS.slice(),currentIndex=0,playing=false,shuffle=true,repeat=false,player=null,ready=false,timer=null;
...
function randomIndex(){return Math.floor(Math.random()*library.length)}
function setMeta(i){const s=library[i];$('title').textContent=s.title;$('artist').textContent=s.artist;$('stationStatus').textContent=`${playing?'ON AIR':'READY'} · DARD-E-DRIVER · ${s.mood}`;$('queueCount').textContent=String(library.length).padStart(2,'0');renderQueue()}
function renderQueue(){const el=$('list');el.innerHTML=library.map((s,i)=>`<button class="queue-item ${i===currentIndex?'active':''}" data-index="${i}"><span>${String(i+1).padStart(2,'0')}</span><div class="qcover">♪</div><div><b class="qtitle">${s.title}</b><div class="qartist">${s.artist}</div></div><span class="qtime">${i===currentIndex?'ON AIR':'READY'}</span><i>▶</i></button>`).join('');el.querySelectorAll('.queue-item').forEach(b=>b.onclick=()=>playIndex(Number(b.dataset.index)))}
function playIndex(i){if(!player||!ready||!library.length)return;currentIndex=(i+library.length)%library.length;playing=true;setMeta(currentIndex);player.loadVideoById({videoId:library[currentIndex].id,startSeconds:0});player.playVideo()}
function tuneIn(){if(!library.length)return;playIndex(shuffle?randomIndex():currentIndex);$('player').scrollIntoView({behavior:'smooth',block:'center'})}
function nextSong(){if(!player||!ready||!library.length)return;let i=repeat?currentIndex:(shuffle?randomIndex():(currentIndex+1)%library.length);if(shuffle&&!repeat&&library.length>1&&i===currentIndex)i=(i+1)%library.length;playIndex(i)}
function prevSong(){playIndex(currentIndex-1)}
function setMood(mood){const matches=SONGS.filter(s=>s.mood.toLowerCase().includes(mood.toLowerCase()));library=matches.length?matches:SONGS.slice();currentIndex=0;shuffle=true;$('shuffle').classList.add('on');renderQueue();$('queueCount').textContent=String(library.length).padStart(2,'0');$('stationStatus').textContent=`${mood.toUpperCase()} · READY TO DRIVE`;tuneIn()}
function onYouTubeIframeAPIReady(){player=new YT.Player('yt',{height:'200',width:'200',videoId:SONGS[0].id,playerVars:{playsinline:1,controls:0,rel:0,iv_load_policy:3},events:{onReady:e=>{ready=true;e.target.setVolume(Number($('volume').value)||80);currentIndex=0;playing=false;setMeta(0)},onStateChange:e=>{if(e.data===YT.PlayerState.PLAYING){playing=true;$('play').textContent='❚❚';$('stationStatus').textContent=`ON AIR · DARD-E-DRIVER · ${library[currentIndex].mood}`;startProgress()}if(e.data===YT.PlayerState.PAUSED){playing=false;$('play').textContent='▶';stopProgress()}if(e.data===YT.PlayerState.ENDED)nextSong()},onError:()=>{$('stationStatus').textContent='TRACK UNAVAILABLE · SKIPPING…';setTimeout(()=>{if(library.length>1)nextSong()},500)}}})}
...
window.addEventListener('load',()=>{renderQueue();document.querySelectorAll('.mood-grid button').forEach((b,i)=>b.addEventListener('click',()=>{const labels=['Road Radio','Dard','90s Romance','Romance'];setMood(labels[i]||'Road Radio')}))});